        ...
        <div style={{ flex: 1, overflowY: "auto", padding: isMobile ? 10 : 14 }}>
          ...
          {pg === "stock" && <div>
            ...
            {detail && detail.price && <div>
              ...
              {detail.fundamental_analysis && !detail.fundamental_analysis.error && (function() {
                var fa = detail.fundamental_analysis;
                var sc = fa.strength_score || 50;
                var gc = sc >= 70 ? gn : sc >= 50 ? gd : rd;
                return <div style={{ background: cd, borderRadius: 10, border: "1px solid " + bd, padding: isMobile ? 12 : 16, marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10, gap: 8, flexWrap: "wrap" }}>
                    <div>
                      <div style={{ fontSize: 10, color: dm }}>🏛 التحليل المؤسسي</div>
                      <div style={{ fontSize: isMobile ? 16 : 20, fontWeight: 700, color: gc }}>{fa.grade_label} - {fa.sector}</div>
                      <div style={{ fontSize: 11, color: "#cbd5e1", marginTop: 2 }}>{fa.business_model}</div>
                    </div>
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: isMobile ? 28 : 36, fontWeight: 800, color: gc, lineHeight: 1 }}>{sc}</div>
                      <div style={{ fontSize: 9, color: dm }}>/100</div>
                    </div>
                  </div>

                  {fa.mode === "heuristic" && <div style={{ background: or + "10", border: "1px solid " + or + "30", borderRadius: 6, padding: "6px 10px", fontSize: 10, color: or, marginBottom: 10 }}>⚠️ وضع استنتاجي - بيانات أساسية كاملة (P/E, ROE) غير متوفرة في خطة Marketstack Basic</div>}

                  <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, 1fr)", gap: 6, marginBottom: 10 }}>
                    <div style={{ background: nv, borderRadius: 6, padding: "6px 8px" }}>
                      <div style={{ fontSize: 9, color: dm }}>الاستقرار</div>
                      <div style={{ fontSize: 12, fontWeight: 700, color: fa.price_stability.score >= 70 ? gn : fa.price_stability.score >= 50 ? gd : rd }}>{fa.price_stability.score}/100</div>
                      <div style={{ fontSize: 9, color: dm }}>{fa.price_stability.label}</div>
                    </div>
                    <div style={{ background: nv, borderRadius: 6, padding: "6px 8px" }}>
                      <div style={{ fontSize: 9, color: dm }}>الزخم 60ي</div>
                      <div style={{ fontSize: 12, fontWeight: 700, color: fa.long_term_momentum.score >= 70 ? gn : fa.long_term_momentum.score >= 50 ? gd : rd }}>{fa.long_term_momentum.score}/100</div>
                      <div style={{ fontSize: 9, color: dm }}>{fa.long_term_momentum.return_60d_pct !== null ? (fa.long_term_momentum.return_60d_pct > 0 ? "+" : "") + fa.long_term_momentum.return_60d_pct + "%" : "-"}</div>
                    </div>
                    <div style={{ background: nv, borderRadius: 6, padding: "6px 8px" }}>
                      <div style={{ fontSize: 9, color: dm }}>السيولة</div>
                      <div style={{ fontSize: 12, fontWeight: 700, color: fa.liquidity.score >= 70 ? gn : fa.liquidity.score >= 50 ? gd : rd }}>{fa.liquidity.score}/100</div>
                      <div style={{ fontSize: 9, color: dm }}>{fa.liquidity.label}</div>
                    </div>
                    <div style={{ background: nv, borderRadius: 6, padding: "6px 8px" }}>
                      <div style={{ fontSize: 9, color: dm }}>التقييم</div>
                      <div style={{ fontSize: 12, fontWeight: 700, color: fa.valuation.range_position < 0.4 ? gn : fa.valuation.range_position < 0.7 ? gd : rd }}>{fa.valuation.range_position !== undefined ? Math.round(fa.valuation.range_position * 100) + "%" : "-"}</div>
                      <div style={{ fontSize: 9, color: dm }}>من المدى السنوي</div>
                    </div>
                  </div>

                  <div style={{ background: nv, borderRadius: 6, padding: 10, marginBottom: 8 }}>
                    <div style={{ fontSize: 10, color: dm, marginBottom: 4 }}>💼 الميزة التنافسية</div>
                    <div style={{ fontSize: 11, color: "#cbd5e1" }}>{fa.competitive_moat}</div>
                  </div>

                  <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 8 }}>
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 600, color: gn, marginBottom: 4 }}>📈 سيناريو متفائل (Bull)</div>
                      {fa.bull_case.slice(0, 4).map(function(p, i) { return <div key={i} style={{ fontSize: 10, color: "#cbd5e1", padding: "3px 0", borderBottom: "1px solid " + bd }}>✓ {p}</div>; })}
                    </div>
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 600, color: rd, marginBottom: 4 }}>📉 سيناريو متشائم (Bear)</div>
                      {fa.bear_case.slice(0, 4).map(function(p, i) { return <div key={i} style={{ fontSize: 10, color: "#cbd5e1", padding: "3px 0", borderBottom: "1px solid " + bd }}>⚠ {p}</div>; })}
                    </div>
                  </div>
                </div>;
              })()}
              ...
              {detail.technical_analysis && !detail.technical_analysis.error && (function() {
                var ta = detail.technical_analysis;
                var sc = ta.momentum_score || 50;
                var gc = sc >= 70 ? gn : sc >= 50 ? gd : rd;
                var trendColor = ta.trend === "bullish" ? gn : ta.trend === "bearish" ? rd : gd;
                return <div style={{ background: cd, borderRadius: 10, border: "1px solid " + bd, padding: isMobile ? 12 : 16, marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10, gap: 8, flexWrap: "wrap" }}>
                    <div>
                      <div style={{ fontSize: 10, color: dm }}>📊 التحليل الفني</div>
                      <div style={{ fontSize: isMobile ? 16 : 20, fontWeight: 700, color: trendColor }}>{ta.trend_label} · قوة {ta.trend_strength}%</div>
                      <div style={{ fontSize: 11, color: "#cbd5e1", marginTop: 2 }}>{ta.summary}</div>
                    </div>
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: isMobile ? 28 : 36, fontWeight: 800, color: gc, lineHeight: 1 }}>{sc}</div>
                      <div style={{ fontSize: 9, color: dm }}>درجة الزخم</div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, 1fr)", gap: 6, marginBottom: 10 }}>
                    <div style={{ background: nv, borderRadius: 6, padding: "6px 8px" }}>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 700, color: ta.rsi > 70 ? rd : ta.rsi < 30 ? gn : gd }}>{ta.rsi !== null ? ta.rsi : "-"}</div>
                      <div style={{ fontSize: 9, color: dm }}>{ta.rsi_status}</div>
                    </div>
                    <div style={{ background: nv, borderRadius: 6, padding: "6px 8px" }}>
                      <div style={{ fontSize: 9, color: dm }}>MACD</div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: ta.macd_signal === "إيجابي" ? gn : rd }}>{ta.macd_data.hist !== null ? ta.macd_data.hist : "-"}</div>
                      <div style={{ fontSize: 9, color: dm }}>{ta.macd_signal}</div>
                    </div>
                    <div style={{ background: nv, borderRadius: 6, padding: "6px 8px" }}>
                      <div style={{ fontSize: 9, color: dm }}>MA20/MA50</div>
                      <div style={{ fontSize: 12, fontWeight: 700 }}>{ta.moving_averages.sma20 || "-"} / {ta.moving_averages.sma50 || "-"}</div>
                      <div style={{ fontSize: 9, color: dm }}>السعر {ta.moving_averages.price_vs_sma50} MA50</div>
                    </div>
                    <div style={{ background: nv, borderRadius: 6, padding: "6px 8px" }}>
                      <div style={{ fontSize: 9, color: dm }}>ATR (14)</div>
                      <div style={{ fontSize: 14, fontWeight: 700 }}>{ta.atr || "-"}</div>
                      <div style={{ fontSize: 9, color: dm }}>تذبذب يومي</div>
                    </div>
                  </div>

                  <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 8, marginBottom: 10 }}>
                    <div style={{ background: nv, borderRadius: 6, padding: 10 }}>
                      <div style={{ fontSize: 10, color: gn, marginBottom: 4 }}>🟢 الدعم</div>
                      {ta.support.length > 0 ? ta.support.map(function(s, i) {
                        return <span key={i} style={{ fontSize: 12, fontWeight: 700, color: gn, marginLeft: 8 }}>{s}</span>;
                      }) : <span style={{ fontSize: 11, color: dm }}>لم يتم تحديد مستويات</span>}
                    </div>
                    <div style={{ background: nv, borderRadius: 6, padding: 10 }}>
                      <div style={{ fontSize: 10, color: rd, marginBottom: 4 }}>🔴 المقاومة</div>
                      {ta.resistance.length > 0 ? ta.resistance.map(function(r, i) {
                        return <span key={i} style={{ fontSize: 12, fontWeight: 700, color: rd, marginLeft: 8 }}>{r}</span>;
                      }) : <span style={{ fontSize: 11, color: dm }}>لم يتم تحديد مستويات</span>}
                    </div>
                  </div>

                  {ta.patterns && ta.patterns.length > 0 && <div style={{ background: nv, borderRadius: 6, padding: 10, marginBottom: 10 }}>
                    <div style={{ fontSize: 10, color: dm, marginBottom: 4 }}>🔍 الأنماط المكتشفة</div>
                    {ta.patterns.map(function(p, i) {
                      return <span key={i} style={{ fontSize: 10, color: p.type === "bullish" ? gn : rd, background: (p.type === "bullish" ? gn : rd) + "15", border: "1px solid " + (p.type === "bullish" ? gn : rd) + "40", borderRadius: 10, padding: "3px 8px", marginLeft: 6 }}>{p.name}</span>;
                    })}
                  </div>}

                  {ta.trade_setup && ta.trade_setup.action !== "انتظار" && <div style={{ background: gd + "10", borderRadius: 6, border: "1px solid " + gd + "30", padding: 10 }}>
                    <div style={{ fontSize: 10, color: dm, marginBottom: 6 }}>🎯 خطة الصفقة</div>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6, fontSize: 11 }}>
                      <div>
                        <div style={{ color: dm, fontSize: 9 }}>الإجراء</div>
                        <div style={{ fontWeight: 700, color: ta.trade_setup.action.indexOf("شراء") >= 0 ? gn : ta.trade_setup.action.indexOf("بيع") >= 0 ? rd : gd }}>{ta.trade_setup.action}</div>
                      </div>
                      <div>
                        <div style={{ color: dm, fontSize: 9 }}>دخول</div>
                        <div style={{ fontWeight: 700 }}>{ta.trade_setup.entry || "-"}</div>
                      </div>
                      <div>
                        <div style={{ color: dm, fontSize: 9 }}>وقف خسارة</div>
                        <div style={{ fontWeight: 700, color: rd }}>{ta.trade_setup.stop_loss || "-"}</div>
                      </div>
                      <div>
                        <div style={{ color: dm, fontSize: 9 }}>الأهداف</div>
                        <div style={{ fontWeight: 700, color: gn }}>{ta.trade_setup.targets.join(" / ") || "-"}</div>
                      </div>
                    </div>
                    {ta.trade_setup.risk_reward && <div style={{ fontSize: 10, color: dm, marginTop: 6 }}>نسبة المخاطرة/العائد: <strong style={{ color: ta.trade_setup.risk_reward >= 2 ? gn : gd }}>1:{ta.trade_setup.risk_reward}</strong></div>}
                  </div>}
                </div>;
              })()}
